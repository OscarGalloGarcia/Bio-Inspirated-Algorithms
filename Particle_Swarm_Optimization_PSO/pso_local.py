# Algoritmo: Particle Swarm Optimization Local Best (lbest PSO)
...
def pso_local(funcion_objetivo, tam_poblacion, dimension, limites,
              max_iter, w, c1, c2, maximizar=False, tam_vecindario=3):
    ...
    for _ in range(max_iter):
        ...
        for i in range(tam_poblacion):
            vecinos = [(i + k) % tam_poblacion for k in range(-radio, radio + 1)]
            if maximizar:
                idx_local = max(vecinos, key=lambda j: f_y[j])
            else:
                idx_local = min(vecinos, key=lambda j: f_y[j])
            y_local = y[idx_local]
            ...
            for j in range(dimension):
                ...
                v[i][j] = (w * v[i][j]
                           + c1 * r1 * (y[i][j] - x[i][j])
                           + c2 * r2 * (y_local[j] - x[i][j]))
                x[i][j] = x[i][j] + v[i][j]
                x[i][j] = max(limites[j][0], min(limites[j][1], x[i][j]))

    if maximizar:
        idx_mejor = f_y.index(max(f_y))
    else:
        idx_mejor = f_y.index(min(f_y))
    return y[idx_mejor]
...
def correr(funcion, dim, limites, maximizar, nombre, archivo=None):
    if dim <= 2:
        ...
        tam_pob = 100
        max_it = 1000
    sol = pso_local(funcion, tam_pob, dim, limites, max_it, 0.7, 1.5, 1.5, maximizar)
    valor = funcion(sol)
...
correr(esfera,     2, [(-5, 5), (-5, 5)],                False, 'Esfera 2D',     'PSO_local_min_esfera.png')
correr(rastrigin,  2, [(-5.12, 5.12), (-5.12, 5.12)],   False, 'Rastrigin 2D',  'PSO_local_min_rastrigin.png')
correr(rosenbrock, 2, [(-2, 2), (-2, 2)],                False, 'Rosenbrock 2D', 'PSO_local_min_rosenbrock.png')

print('==================== MAXIMIZACION 2D ====================')
correr(neg_esfera,  2, [(-5, 5), (-5, 5)],                           True, 'Esfera negativa 2D', 'PSO_local_max_neg_esfera.png')
correr(seno_coseno, 2, [(-math.pi, math.pi), (-math.pi, math.pi)],   True, 'sin(x1)*cos(x2)',    'PSO_local_max_seno_coseno.png')
correr(easom,       2, [(0, 6), (0, 6)],                             True, 'Easom 2D',            'PSO_local_max_easom.png')
